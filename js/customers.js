...
(function () {
  ...
  function personaHtml(p, profile) {
    var traits = Object.keys(p.traits).map(function (k) { return '<tr><td>' + esc(k) + '</td><td>' + p.traits[k] + ' of 100</td></tr>'; }).join('');
    return '<html xmlns:o="urn:schemas-microsoft-office:office" xmlns:w="urn:schemas-microsoft-office:office:word" xmlns="http://www.w3.org/TR/REC-html40">'
      ...
      + '<p style="color:#666;margin:2px 0 10px">Prepared by Clarity' + (profile && profile.name ? ' · ' + esc(profile.name) : '') + '</p>'
  ...
  var SECTOR_CTX = {
    ...
    retail:   { name: 'independent retail',     market: 'a crowded but navigable direct-to-consumer market', price: '$25–35 per order' },
  ...
  var REP = {
    ...
    status: { dq: 79, profileExtra: 'They buy for identity and signal — being seen matters as much as the product.',
      ...
      wtp: { band: 'Mid-high — pays for cachet', note: 'Pays for prestige and aesthetics; perceived exclusivity raises what they’ll pay. Benchmark {price}.' },
  ...
  function buildAudience(profile, name, archId) {
    ...
    p.report = {
      ...
      evidence: AUDIENCE_EVIDENCE, sources: AUDIENCE_EVIDENCE.length, depth: 'Standard (3 searches)'
    };
  ...
  function ClarityCustomersMission(props) {
          ...
          : e('div', { className: 'cu-roster' },
              sketches.map(function (p, i) {
                return e('button', { key: i, className: 'cu-rostercard' + (p.name === primary ? ' primary' : ''), onClick: function () { setCurrent(p); setView('result'); } },
                  p.name === primary && e('span', { className: 'cu-primary-badge' }, 'Main person'),
                  DossierCard(p, true));
              })),
    ...
    return shell(e(React.Fragment, null,
      ...
      r && e('div', { className: 'rc-doc rc-' + theme, style: { '--rc-accent': CATEGORY.accent, '--rc-accent-dim': CATEGORY.accentDim } },
        ...
        ].filter(Boolean), storeKey: 'customers:' + ((p && p.name) || 'persona') })
      ),
